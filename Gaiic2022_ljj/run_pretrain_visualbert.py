...
def read_data(args, tokenizer: BertTokenizer) -> dict:
    train_fine = os.path.join(args.data_path, 'train_fine.txt')
    train_coarse = os.path.join(args.data_path, 'train_coarse.txt')
    
    inputs = defaultdict(list)
    
    if args.debug:
        with open(train_fine, 'r') as f:
            for i, data in enumerate(tqdm(f)):
                if i == 5000:
                    break
                data = json.loads(data)
                inputs_dict = tokenizer.encode_plus(data['title'], add_special_tokens=True,
                                                return_token_type_ids=True, return_attention_mask=True)

                inputs['input_ids'].append(inputs_dict['input_ids'])
                inputs['token_type_ids'].append(inputs_dict['token_type_ids'])
                inputs['attention_mask'].append(inputs_dict['attention_mask'])
                inputs['visual_embeds'].append(data['feature'])
                if i==0:
                    print('\n')
                    print(inputs_dict['token_type_ids'], type(inputs_dict['token_type_ids']))
                    print(data['feature'], type(data['feature']))
        ...
        with open(train_fine, 'r') as f:
            for i, data in enumerate(tqdm(f)):
                ...
                inputs['token_type_ids'].append(inputs_dict['token_type_ids'])
                inputs['attention_mask'].append(inputs_dict['attention_mask'])
                inputs['visual_embeds'].append(torch.tensor(data['feature']))


        with open(train_coarse, 'r') as f:
            for i, data in enumerate(tqdm(f)):
                data = json.loads(data)
                inputs_dict = tokenizer.encode_plus(data['title'], add_special_tokens=True,
                                                return_token_type_ids=True, return_attention_mask=True)
...
class DGDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple:
        data = (self.data_dict['input_ids'][index],
                ...
                self.data_dict['visual_embeds'][index])
...
class DGDataCollator:
    ...
    def pad_and_truncate(self, input_ids_list, token_type_ids_list,
                         attention_mask_list, visual_embeds_list, max_seq_len):
        input_ids = torch.zeros((len(input_ids_list), max_seq_len), dtype=torch.long)
        token_type_ids = torch.zeros_like(input_ids)
        attention_mask = torch.zeros_like(input_ids)
        visual_embeds = torch.zeros((len(visual_embeds_list), len(visual_embeds_list[0])), dtype=torch.float32)
        ...
        for i in range(len(input_ids_list)):
            ...
            visual_embeds[i] = torch.tensor(visual_embeds_list[i], dtype=torch.float32)
            
        return input_ids, token_type_ids, attention_mask, visual_embeds
    ...
    def mask_tokens(
        ...
        # visualbert 
        p = torch.full((labels.shape[0], 1), -100)#.unsqueeze(1)
        labels = torch.cat((labels, p), 1)
        
        return inputs, labels

    def __call__(self, examples: list) -> dict:
        input_ids_list, token_type_ids_list, attention_mask_list, visual_embeds_list = list(zip(*examples))
        cur_max_seq_len = max(len(input_id) for input_id in input_ids_list)
        max_seq_len = min(cur_max_seq_len, self.max_seq_len)

        input_ids, token_type_ids, attention_mask, visual_embeds = self.pad_and_truncate(input_ids_list,
                                                                                              token_type_ids_list,
                                                                                              attention_mask_list,
                                                                                              visual_embeds_list,
                                                                                              max_seq_len)
...
def build_model_and_tokenizer(args):
    ...
    ## bert ernie bert_wwm bert_wwwm_ext
        'bert-base-chinese': (BertConfig, AutoModelForMaskedLM, BertTokenizer),
        'nezha-cn-base': (NeZhaConfig, NeZhaForMaskedLM, BertTokenizer),
        'VisualBert': (BertConfig, VisualBertForPreTraining, BertTokenizer),
    }
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_name]
    
    tokenizer = tokenizer_class.from_pretrained(args.pretrain_model_path)
    model_config = config_class.from_pretrained(args.pretrain_model_path)
    model = model_class.from_pretrained(pretrained_model_name_or_path=args.pretrain_model_path,
                                             config=model_config)
...
def pretrain(args):
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for step, batch in enumerate(train_iterator):
            ...
            batch_cuda['visual_embeds'] = batch_cuda['visual_embeds'].unsqueeze(1)
            # batch_cuda['visual_embeds']的大小是[bs, 1, seq_length], batch_cuda['input_ids']的大小是[bs, seq_length]
            batch_cuda.update(
                { 
                    "visual_token_type_ids":torch.ones(batch_cuda['visual_embeds'].shape[:-1], dtype=torch.long).to(args.device),
                    "visual_attention_mask":torch.ones(batch_cuda['visual_embeds'].shape[:-1], dtype=torch.float).to(args.device)
                }
            )
...
def main():
    ...
    parser.add_argument('--model_name', type=str,
                        default=f'VisualBert')
    parser.add_argument('--pretrain_model_path', type=str, # VisualBert  nezha-cn-base
                        default=f'./prev_trained_model/VisualBert')
    parser.add_argument('--data_cache_path', type=str,
                        default=f'./process_data/pkl/pretrain.pkl')
#     parser.add_argument('--vocab_path', type=str,
#                         default=f'./prev_trained_model/nezha-cn-base/vocab.txt')
    parser.add_argument('--save_path', type=str,
                        default='./new_self_pretrained_model')
    parser.add_argument('--record_save_path', type=str,
                        default='./new_self_pretrained_model_record')
