...
def evaluate(df, CFG, fold):
    ...
    model.eval()
    model.load_state_dict(torch.load(f'{CFG.out_dir}/model_roberta_fold{fold}.pth'))
...
CFG.tokenizer_path = '../../data/contest_data/pretrain_model/nezha-cn-base'
CFG.model_path = '../../data/contest_data/pretrain_model/nezha-cn-base'
...
def evaluate(df, CFG, fold):
    ...
    model = Model(CFG).to(device)
    model.eval()
    model.load_state_dict(torch.load(f'{CFG.out_dir}/model_nezha_fold{fold}.pth', map_location='cpu'), strict=False)
    for text, feature in tqdm(loader):
        text = CFG.tokenizer(text, return_tensors='pt', add_special_tokens=True, padding=True)
        for k, v in text.items():
            text[k] = v.cuda()
...
with open(os.path.join(CFG.out_dir, '../../data/submission/sub.txt'), 'w') as f:
    f.writelines(submit)
